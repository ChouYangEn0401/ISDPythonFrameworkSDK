...
> **Lambda 一律被拒絕**：`RegisterEvent` 偵測到 lambda（`__name__ == "<lambda>"`）會立即 `raise TypeError`，而非事後無聲失效。理由：沒有強引用持有者的匿名 closure 會被弱引用立即 GC、無法可靠觸發或解除註冊。請改用 `self.method` 或 module-level / static 具名函式。（注意：此限制僅針對 `SingletonEventManager`；`MulticastCallback` 不受影響。）
...
**錯誤處理（`TriggerEvent`）分兩類，互不混淆：**
- **A 類「訂閱者不在了」**（weakref 回 `None`，持有者被刪 / GC）→ 視為正常生命週期，**安靜丟棄**（`logging.debug`），不保留。
- **B 類「訂閱者壞了」**（callback 活著但 body 拋例外）→ 真 bug，**大聲記錄**（`logging.exception`，含 traceback）但**隔離**：不連累其他 handler、也**不退訂**它。
> 兩類都走標準庫 `logging`（logger 名 `isd_py_framework_sdk.events`），刻意不耦合 `message_logger`。舊版那種「`except TypeError` 吞掉並把 handler 退訂」的行為已移除。
...
- 同一個 callback 只能在弱引用有效時被調用；持有者 GC 後會被安靜清掃（`logging.debug`，A 類）。
- callback 不可為 lambda——`RegisterEvent` 會直接 `raise TypeError`。請用 `self.method` 或具名函式。
- handler 內部拋例外不會中斷其他 handler、也不會害自己被退訂（B 類，`logging.exception` 記錄）。
