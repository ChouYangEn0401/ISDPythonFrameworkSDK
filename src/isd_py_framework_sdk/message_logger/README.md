...
**推薦公開 API：**
- 一般使用者只需要 `SingletonSystemLogger` / `get_logger()` / `configure_logger()` 加上 concrete adapters。
- 自訂輸出時才需要 `LoggerAdapterBase`。
...
# 或淺色主題（適合白色 / 淺色終端）
# logger.add_adapter(LightThemeTerminalAdapter(level_filter="DEBUG"))

logger.info("INFO 訊息")
logger.warning("WARNING 訊息")

# 隨時調整單一 adapter 的過濾等級
adapter = DarkThemeTerminalAdapter(level_filter="DEBUG")
adapter.set_level_filter("WARNING")       # 現在只顯示 WARNING 以上
logger.add_adapter(adapter)
```
...
```python
# 覆寫模式（每次啟動清空檔案）
FileAdapter(level_filter="DEBUG", output_file=Path("debug.log"), mode="w")

# 手動關閉（釋放檔案句柄）
adapter = FileAdapter(level_filter="WARNING", output_file=Path("app.log"))
logger.add_adapter(adapter)
# ... 程式結束前 ...
adapter.close()
```
...
logger = SingletonSystemLogger()
logger.add_adapter(DarkThemeTkinterAdapter(level_filter="INFO", tk_window=text))
...
def set_theme(theme: str):
    ...
    if theme == "dark":
        ...
        logger.add_adapter(LightThemeTkinterAdapter(level_filter="INFO", tk_window=text))
```
...
logger = SingletonSystemLogger()
adapter = DarkThemeTkinterAdapter(level_filter="DEBUG")   # 先不傳 tk_window
logger.add_adapter(adapter)
...
```python
adapter = DarkThemeTkinterAdapter(level_filter="INFO", tk_window=text)
logger.add_adapter(adapter)
...
logger.add_adapter(DarkThemeTerminalAdapter(level_filter="DEBUG"))          # console 顯示全部
logger.add_adapter(FileAdapter(level_filter="WARNING", output_file=Path("app.log")))    # 檔案只寫 WARNING+
# logger.add_adapter(DarkThemeTkinterAdapter(level_filter="INFO", tk_window=text))  # UI 顯示 INFO+
...
logger = SingletonSystemLogger()
logger.add_adapter(SlackAdapter("ERROR", "https://hooks.slack.com/..."))
...
# 移除特定 adapter
logger.remove_adapter(some_adapter)       # 若不存在，靜默忽略
