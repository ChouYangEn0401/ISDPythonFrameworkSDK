...
### 批次 CI 執行器（`tools/run_file_tests.py`）

提供一個 CLI 工具，讀取一份 config（YAML 或 JSON）後批次執行所有比對，並：
- 彩色輸出每一項的 PASS / FAIL
- 列印整體統計（X PASS / Y FAIL）
- 回傳 exit code（0 = 全通過，1 = 任一失敗）→ **可直接接入 CI pipeline**
- 可選輸出 HTML 差異報告

**安裝前置套件（視 config 中使用的格式而定）：**
```bash
pip install isd-python-framework[filetest]
```
...
# 自訂 HTML 標題
python tools/run_file_tests.py suite.yaml --html report.html --title "Sprint 42 回歸測試"

# 第一個失敗立即停止
python tools/run_file_tests.py suite.yaml --fail-fast
```

**Config 格式（`suite.yaml`）：**
```yaml
tests:
  # CSV
  - format: csv
...
> 每個 `tests` 項目只需設定：`format`（支援格式字串）、`label`（顯示名稱），其餘 key 直接作為該格式對應函式的 config 傳入。

**Config 格式（JSON 版本另一種寫法）：**
```json
{
  "tests": [
    { "format": "csv", "label": "My CSV", "target_path": "out.csv", "bench_path": "expected.csv" },
    { "format": "json", "label": "My JSON", "target_path": "out.json", "bench_path": "expected.json" }
  ]
}
```
...
所有比對函式回傳 `CompareResult` 物件（支援 `bool` 判斷，向下相容），可直接送入 `generate_html_report`：
...
r1 = compare_csv_files({
    ...
    "checks":      ["content", "row_count"],
    "mask":        {"exclude_rows": [1]},
})
r2 = compare_json_files({
    "target_path": "output.json",
    "bench_path":  "expected.json",
    "mask":        {"exclude_paths": ["$.timestamp"]},
})

# 生成自包含 HTML 報告（無外部依賴，可直接用瀏覽器開啟）
generate_html_report([r1, r2], "report.html", title="Sprint 42 回歸測試")
```

HTML 報告特色：
- **深色主題**，自包含（單一 `.html` 檔案，無需 CDN 或外部 CSS）
- 頂部顯示 **PASS / FAIL 統計**
- 每項測試可折疊展開（失敗項目預設展開）
- 差異以表格呈現：**路徑 / 位置 | 描述 | 實際值 | 預期值**，顏色標示

向下相容性：
```python
# 原本的 bool 用法仍完全有效
if compare_csv_files(cfg):
    print("PASS")
else:
    print("FAIL")
```
...
```
1. 第一次確認輸出正確後，存成 expected/ 資料夾裡的 bench 檔案
2. 每次版本更新後執行：
      python tools/run_file_tests.py suite.yaml --html report.html
3. exit code 0 = 輸出符合預期；exit code 1 = 有變動，需確認
4. 確認是 intentional change → 更新 bench 檔案並 commit（說明原因）
5. 確認是 bug → 修正程式碼

在 CI（GitHub Actions / GitLab CI）中加入：
      - run: python tools/run_file_tests.py suite.yaml
   當輸出與 bench 不符時，PR 自動被阻擋。
```
